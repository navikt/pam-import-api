...
if [[ $REPLY =~ ^[Yy]$ ]]
then
  curl -POST -H "Authorization: Bearer `cat $PATH_DEV_KEY`" -H "Content-Type: application/json" -d "$json"  "https://pam-import-api.intern.dev.nav.no/stillingsimport/internal/providers"
